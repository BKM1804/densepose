...
def no_face_segment(extractor , pred_boxes, corase_segm, fine_segm, u, v ):
    ...
    for i in range(len(pred_boxes)):
        ext = extractor(pred_boxes[i:i+1], corase_segm[i:i+1], fine_segm[i:i+1], u[i:i+1], v[i:i+1])
        # mask = ext[1] >= 2
        # ext[1][mask] = 0
        rt.append(ext)
    return rt
...
class DenseposeDetector:
    def __init__(self, model , n_part = 24):
    ...
    def __call__(self, input_image, detect_resolution=512, output_type="pil", upscale_method="INTER_CUBIC", cmap="viridis", **kwargs):
        ...
        densepose_results = no_face_segment(extractor , pred_boxes, corase_segm, fine_segm, u, v)
